...
class AFP(EmbeddingBase):
    ...
    def function(self, image: Image, executable: bytes) -> Image:
        ...
        for y in range(height):
            for x in range(width):
                ...
                if data_index + 4 <= len(payload_bits):
                    bits = payload_bits[data_index:data_index + 4]
                    a = (a & ~0b1111) | int(bits, 2)
                    data_index += 4
    ...
    def reverse_function(self, image: Image) -> bytes:
        ...
        try:
            ...
            raise ValueError("Decoded message is not valid hex. Possibly corrupted or incomplete.")

    @override
    def __set_limit_function(self, dims) -> None:
        self.limit = (dims[0] * dims[1] * 4) // 8  # 4 bits per alpha
